...
import { persistPreparedRecord, prepareRecordCommand, PreparedRecordCommand, reversePreparedRecord } from '../../services/recordPersistence';
...
export default function AddEntry() {
  ...
  const handleSave = async (duplicateAcknowledged = false) => {
    ...
    if (!selectedMethod || isDoseMissing()) {
      setHighlightMissing(true);
      return;
    }
  ...
  if (saved) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-6 px-4 text-center">
        ...
        <div>
          <h2 className="text-2xl font-bold text-usnee-text">Сохранено на устройстве</h2>
          <p className="mt-1 text-sm text-usnee-text2">Операция сохранена локально и ждёт отправки.</p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {highlightMissing && isDoseMissing() && (
            <div className="rounded-lg bg-usnee-danger/10 p-3 text-xs text-usnee-danger">
              Укажите положительную дозу или объём, чтобы сохранить запись.
            </div>
          )}
        </div>
      )}
