...
describe('Home components', () => {
  ...
  it('renders no batch and malformed states', () => { const { rerender } = render(<BatchHeroCard onOpenBatch={() => {}} />); expect(screen.getByText('Партии пока нет')).toBeInTheDocument(); rerender(<BatchHeroCard malformed onOpenBatch={() => {}} />); expect(screen.getByText('Не удалось рассчитать остаток')).toBeInTheDocument(); });
  ...
  it('counts only entries in current calendar week', () => { const now = Date.now(); render(<WeeklySummaryCard entries={[entry, { ...entry, id: 'old', timestamp: now - 14 * 86400000 }, { ...entry, id: 'future', timestamp: now + 10 * 86400000 }]} todayCount={1} now={now} />); expect(screen.getByText('1 запись')).toBeInTheDocument(); });
});
