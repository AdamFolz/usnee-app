...
describe('Home components', () => {
  ...
  it('counts only entries in last seven days', () => { const now = Date.now(); render(<WeeklySummaryCard entries={[entry, { ...entry, id: 'old', timestamp: now - 8 * 86400000 }, { ...entry, id: 'future', timestamp: now + 1000 }]} todayCount={1} now={now} />); expect(screen.getByText('1 запись')).toBeInTheDocument(); });
});
