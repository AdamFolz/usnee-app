...
export function QuickActionGrid(props: QuickActionGridProps) {
  ...
  return <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{actions.map(([label, Icon, key], index) => (
    <button key={key} type="button" onClick={callbacks[key]} className="rounded-card text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-usnee-focus">
      <Surface variant="interactive" className={cx('ambient-sheen animate-stagger-in flex min-h-24 flex-col justify-between p-4', key === 'record' && 'border-usnee-brand/40 bg-usnee-brand/15')} style={{ animationDelay: `${index * 70}ms` }}>
